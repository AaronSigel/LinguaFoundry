...
LinguaFoundry uses small, reviewable changes that can be implemented by humans
or coding agents.
...
1. Inspect the repository layout, existing conventions, configuration, and
   nearby files before editing.
...
1. Add or update tests and documentation in proportion to the risk of the
   change.
...
Human reviewers should check that the implementation matches the issue, follows
local conventions, has suitable verification, and does not introduce hidden
scope changes.

## Decision Records

Use ADRs for decisions that are hard to reverse or likely to guide future work.
Store them in `docs/adr/` using the local template.
